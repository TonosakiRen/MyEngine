...
void Transition::CreatePipeline() {
...

	{
		...
		rootparams[int(RootParameter::kSpeed)].InitAsConstants(1, 0);
		rootparams[int(RootParameter::kT)].InitAsConstants(1, 1);
...
void Transition::Draw(ColorBuffer& resultBuffer, DescriptorHandle& textureBuffer, CommandContext* commandContext) {
	...
	// CBVをセット（ワールド行列）
	commandContext->SetConstant(static_cast<UINT>(RootParameter::kSpeed), speed_, 0);
	// CBVをセット（ワールド行列）
	commandContext->SetConstant(static_cast<UINT>(RootParameter::kT), t_, 0);

	// SRVをセット
	commandContext->SetDescriptorTable(static_cast<UINT>(RootParameter::kTexture), textureBuffer);

	commandContext->TransitionResource(resultBuffer, D3D12_RESOURCE_STATE_ALL_SHADER_RESOURCE);
	commandContext->SetDescriptorTable(static_cast<UINT>(RootParameter::kResult), resultBuffer.GetSRV());
