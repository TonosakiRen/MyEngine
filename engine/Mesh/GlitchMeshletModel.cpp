/**
 * @file GlitchMeshletModel.cpp
 * @brief GlitchMeshletでの描画Pipeline
 */
...
float GlitchMeshletModel::t = 0.0f;

void GlitchMeshletModel::Initialize(CommandContext& commandContext) {
    ...
    blockNoise_ = std::make_unique<BlockNoise>();
    blockNoise_->Initialize();
    blockNoise_->Render(commandContext);
}

void GlitchMeshletModel::Finalize()
{
...
void GlitchMeshletModel::PreDraw(PipelineType pipelineType,CommandContext& commandContext, const ViewProjection& viewProjection, const ViewProjection& cullingViewProjection, const TileBasedRendering& tileBasedRendering) {
    ...
    case Renderer::kForward:
        ...
        commandContext.SetConstants(static_cast<UINT>(ForwardRootParameter::kT), t);
        break;
    case Renderer::kDeferred:
...
void GlitchMeshletModel::CreatePipeline() {
...
void GlitchMeshletModel::CreateForwardPipeline()
{
    ComPtr<IDxcBlob> msBlob;
    ComPtr<IDxcBlob> psBlob;
    ComPtr<IDxcBlob> asBlob;
    ...
    psBlob = shaderManager->Compile(L"forwardPlus/GlitchMeshPS.hlsl", ShaderManager::kPixel);
    assert(psBlob != nullptr);
...

    {
        ...
        descRangeSRV[int(ForwardRootParameter::kBlockNoise)].Init(D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 1, 11);
        ...
        rootparams[(int)ForwardRootParameter::kBlockNoise].InitAsDescriptorTable(1, &descRangeSRV[(int)ForwardRootParameter::kBlockNoise]);
        ...
        rootparams[(int)ForwardRootParameter::kT].InitAsConstants(1, 6);
...
void GlitchMeshletModel::Draw(CommandContext& commandContext, uint32_t modelHandle, const WorldTransform& worldTransform, const Material& material,const uint32_t textureHandle) {
    ...
    case Renderer::kForward:
        ...
        for (const auto& mesh : modelItem.meshes) {
            ...
            commandContext.SetDescriptorTable(UINT(ForwardRootParameter::kBlockNoise), blockNoise_->GetResult().GetSRV());
