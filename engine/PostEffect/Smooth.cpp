...
void Smooth::CreatePipeline(ColorBuffer& orignalBuffer) {
...

	{
		...
		rootSignature_.Create(L"smoothRootSignature", rootSignatureDesc);
...

	{
		...
		// グラフィックスパイプラインの生成
		pipelineState_.Create(L"smoothPipeline",gpipeline);
	}
}



void Smooth::Initialize(ColorBuffer& orignalBuffer) {
	CreatePipeline(orignalBuffer);
}

void Smooth::Draw(ColorBuffer& originalBuffer, ColorBuffer& tmpBuffer, CommandContext& commandContext) {
