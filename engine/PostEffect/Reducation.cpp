...
void Reducation::CreatePipeline() {
	...
	psBlob = shaderManager->Compile(L"ReducationPS.hlsl", ShaderManager::kPixel);
...

	{
		...
		gpipeline.RTVFormats[0] = resultBuffer_.GetFormat(); // 0～255指定のRGBA
...
void Reducation::Draw(ColorBuffer& originalBuffer, CommandContext& commandContext) {
