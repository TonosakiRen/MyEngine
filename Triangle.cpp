...
/// 静的メンバ変数の実体
/// </summary>
DirectXCommon* Triangle::sDirectXCommon = nullptr;
UINT Triangle::sDescriptorHandleIncrementSize = 0;
ID3D12GraphicsCommandList* Triangle::sCommandList = nullptr;
ComPtr<ID3D12RootSignature> Triangle::sRootSignature;
ComPtr<ID3D12PipelineState> Triangle::sPipelineState;

void Triangle::StaticInitialize(int window_width, int window_height) {
...
void Triangle::PreDraw(ID3D12GraphicsCommandList* commandList) {
    ...
    assert(Triangle::sCommandList == nullptr);
...
Triangle* Triangle::Create() {
    // 3Dオブジェクトのインスタンスを生成
    Triangle* object3d = new Triangle();
    assert(object3d);
...
void Triangle::CreateMesh() {
    ...
    vertices_[1].pos = {0.0f,0.5f,0.0f};
    vertices_[2].pos = {0.5f,-0.5f,0.0f };
    vertices_[0].uv = { 0.0f,1.0f };
    vertices_[1].uv = { 0.5f,0.0f };
    vertices_[2].uv = { 1.0f,1.0f };
    vertices_[0].normal = { 0.0f,0.0f,-1.0 };
    vertices_[1].normal = { 0.0f,0.0f,-1.0 };
    vertices_[2].normal = { 0.0f,0.0f,-1.0 };
...
void Triangle::Draw(const WorldTransform& worldTransform, const ViewProjection& viewProjection, const DirectionalLight& directionalLight, const Material& material, uint32_t textureHadle) {
    // nullptrチェック
    assert(sDirectXCommon->GetDevice());
    assert(sCommandList);
