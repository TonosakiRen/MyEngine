...
/// 静的メンバ変数の実体
/// </summary>
DirectXCommon* Sphere::sDirectXCommon = nullptr;
UINT Sphere::sDescriptorHandleIncrementSize = 0;
ID3D12GraphicsCommandList* Sphere::sCommandList = nullptr;
ComPtr<ID3D12RootSignature> Sphere::sRootSignature;
ComPtr<ID3D12PipelineState> Sphere::sPipelineState;
ComPtr<ID3D12PipelineState> Sphere::sShadowPipelineState;
ComPtr<ID3D12Resource> Sphere::sShadowMapRenderTargetResource;
ComPtr<ID3D12Resource> Sphere::sShadowMapDepthStencilResource;
uint32_t  Sphere::shadowMapWidth;
uint32_t  Sphere::shadowMapHeight;
Sphere::RenderPass Sphere::currentRenderPass = Sphere::NonePass;

void Sphere::StaticInitialize(uint32_t shadowMap_width, uint32_t shadowMap_height) {
...
void Sphere::BeginPass(ID3D12GraphicsCommandList* commandList, Sphere::RenderPass pass) {
	// PreDrawとPostDrawがペアで呼ばれていなければエラー
	assert(Sphere::sCommandList == nullptr);
	assert(currentRenderPass == Sphere::NonePass);
...
void Sphere::InitializeGraphicsPipelines() {
...
void Sphere::CreateShadowMapRenderTarget() {
	//RTVの設定
...
void Sphere::CreateShadowMapDepthStencil() {
	//DSVの設定
...
void Sphere::InitializeDefaultGraphicsPipelines() {
...
void Sphere::InitializeShadowGraphicsPipelines() {
	HRESULT result = S_FALSE;
	ComPtr<IDxcBlob> vsBlob;    // 頂点シェーダオブジェクト
	ComPtr<IDxcBlob> psBlob;    // ピクセルシェーダオブジェクト
...
void Sphere::CreateMesh() {
	...
	vertices_.resize(kSubdivision * kSubdivision * 6);
	indices_.resize(kSubdivision * kSubdivision * 6);

	const float kLonEvery = float(2.0f * std::numbers::pi_v<float> / kSubdivision);//経度分割一つ分の角度
	const float kLatEvery = float(std::numbers::pi_v<float> / kSubdivision);//緯度分割一つ分の角度
	//緯度の方向に分割 -π/2 ～	π/2
	for (uint32_t latIndex = 0; latIndex < kSubdivision; ++latIndex) {
		...
		for (uint32_t lonIndex = 0; lonIndex < kSubdivision; ++lonIndex) {
			...
			//頂点にデータを入力する。基準点a
			vertices_[start].pos.x = std::cosf(lat) * std::cosf(lon);
			vertices_[start].pos.y = std::sin(lat);
			vertices_[start].pos.z = std::cosf(lat) * std::sinf(lon);
			vertices_[start].uv = { float(lonIndex) / float(kSubdivision),1.0f - float(latIndex) / kSubdivision };
			vertices_[start].normal.x = vertices_[start].pos.x;
			vertices_[start].normal.y = vertices_[start].pos.y;
			vertices_[start].normal.z = vertices_[start].pos.z;
			indices_[start] = start;

			vertices_[start + 1].pos.x = std::cosf(lat + kLatEvery) * std::cosf(lon);
			vertices_[start + 1].pos.y = std::sin(lat + kLatEvery);
			vertices_[start + 1].pos.z = std::cosf(lat + kLatEvery) * std::sinf(lon);
			vertices_[start + 1].uv = { float(lonIndex) / float(kSubdivision),1.0f - float(latIndex + 1) / kSubdivision };
			vertices_[start + 1].normal.x = vertices_[start + 1].pos.x;
			vertices_[start + 1].normal.y = vertices_[start + 1].pos.y;
			vertices_[start + 1].normal.z = vertices_[start + 1].pos.z;
			indices_[start + 1] = start + 1;

			vertices_[start + 2].pos.x = std::cosf(lat) * std::cosf(lon + kLonEvery);
			vertices_[start + 2].pos.y = std::sin(lat);
			vertices_[start + 2].pos.z = std::cosf(lat) * std::sinf(lon + kLonEvery);
			vertices_[start + 2].uv = { float(lonIndex + 1) / float(kSubdivision),1.0f - float(latIndex) / kSubdivision };
			vertices_[start + 2].normal.x = vertices_[start + 2].pos.x;
			vertices_[start + 2].normal.y = vertices_[start + 2].pos.y;
			vertices_[start + 2].normal.z = vertices_[start + 2].pos.z;
			indices_[start + 2] = start + 2;

			vertices_[start + 3].pos.x = std::cosf(lat) * std::cosf(lon + kLonEvery);
			vertices_[start + 3].pos.y = std::sin(lat);
			vertices_[start + 3].pos.z = std::cosf(lat) * std::sinf(lon + kLonEvery);
			vertices_[start + 3].uv = { float(lonIndex + 1) / float(kSubdivision),1.0f - float(latIndex) / kSubdivision };
			vertices_[start + 3].normal.x = vertices_[start + 3].pos.x;
			vertices_[start + 3].normal.y = vertices_[start + 3].pos.y;
			vertices_[start + 3].normal.z = vertices_[start + 3].pos.z;
			indices_[start + 3] = start + 3;

			vertices_[start + 4].pos.x = std::cosf(lat + kLatEvery) * std::cosf(lon);
			vertices_[start + 4].pos.y = std::sin(lat + kLatEvery);
			vertices_[start + 4].pos.z = std::cosf(lat + kLatEvery) * std::sinf(lon);
			vertices_[start + 4].uv = { float(lonIndex) / float(kSubdivision),1.0f - float(latIndex + 1) / kSubdivision };
			vertices_[start + 4].normal.x = vertices_[start + 4].pos.x;
			vertices_[start + 4].normal.y = vertices_[start + 4].pos.y;
			vertices_[start + 4].normal.z = vertices_[start + 4].pos.z;
			indices_[start + 4] = start + 4;

			vertices_[start + 5].pos.x = std::cosf(lat + kLatEvery) * std::cosf(lon + kLonEvery);
			vertices_[start + 5].pos.y = std::sin(lat + kLatEvery);
			vertices_[start + 5].pos.z = std::cosf(lat + kLatEvery) * std::sinf(lon + kLonEvery);
			vertices_[start + 5].uv = { float(lonIndex + 1) / float(kSubdivision),1.0f - float(latIndex + 1) / kSubdivision };
			vertices_[start + 5].normal.x = vertices_[start + 5].pos.x;
			vertices_[start + 5].normal.y = vertices_[start + 5].pos.y;
			vertices_[start + 5].normal.z = vertices_[start + 5].pos.z;
			indices_[start + 5] = start + 5;
...
void Sphere::Draw(const WorldTransform& worldTransform, const ViewProjection& viewProjection, const DirectionalLight& directionalLight, const Material& material, const ViewProjection& shadowViewProjection, uint32_t textureHadle) {
	// nullptrチェック
	assert(sDirectXCommon->GetDevice());
	assert(sCommandList);
...
void Sphere::DrawShadow(const WorldTransform& worldTransform, const ViewProjection& viewProjection)
{
	// nullptrチェック
	assert(sDirectXCommon->GetDevice());
	assert(sCommandList);
